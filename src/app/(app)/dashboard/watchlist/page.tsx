import { redirect } from "next/navigation";
import { AddToWatchlistForm } from "@/components/add-to-watchlist-form";
import { WatchlistTable } from "@/components/watchlist-table";
import { getSessionUser } from "@/lib/auth";
import { getDictionary, getLocale } from "@/lib/i18n";
import { listWatchlistWithRatings } from "@/lib/tickers";
import type { Verdict } from "@/lib/ratings";

export const dynamic = "force-dynamic";

export default async function WatchlistPage() {
  const user = await getSessionUser();
  if (!user) redirect("/login");

  const locale = await getLocale();
  const w = getDictionary(locale).memberPages.watchlist;
  const items = await listWatchlistWithRatings(user.id);

  const rows = items.map((t) => ({
    symbol: t.symbol,
    name: t.name,
    exchange: t.exchange,
    quant_score: t.quant_score,
    ops_verdict: (t.ops_verdict as Verdict | null) ?? null,
    rank_overall: t.rank_overall,
  }));

  return (
    <div className="mx-auto w-full max-w-[960px] px-4 py-6 md:px-6">
      <header className="mb-4 border-b border-border pb-3">
        <span className="label-caps">{w.label}</span>
        <h1 className="mt-1 text-2xl font-bold text-foreground">{w.title}</h1>
        <p className="mt-1 text-[13px] text-muted">{w.subtitle}</p>
      </header>

      <div className="mb-4">
        <AddToWatchlistForm />
      </div>

      {rows.length === 0 ? (
        <section className="card">
          <p className="py-12 text-center text-[13px] text-muted">{w.empty}</p>
        </section>
      ) : (
        <WatchlistTable rows={rows} />
      )}
    </div>
  );
}
