...
export default async function MarketingHome() {
  ...
  const m = t.marketing;
  const focus = investmentFocusLocalized(locale);

  const [user, linkage] = await Promise.all([getSessionUser(), getValueChainLinkage()]);
  const ctaLabel = user ? m.ctaOpen : m.ctaEnter;
    ...
    <main style={{ background: theme.bg, color: theme.ink, fontFamily: '"Noto Serif SC", "Songti SC", Georgia, serif' }}>
      ...
      >
        <div className="mx-auto flex max-w-[1200px] items-center justify-between px-6 py-4">
          ...
          >
            <a href="#focus" className="hover:opacity-70">{m.navFocus}</a>
            <a href="#value-chain" className="hover:opacity-70">{m.navValueChain}</a>
            <a href="#philosophy" className="hover:opacity-70">{m.navPhilosophy}</a>
            <a href="#alpha" className="hover:opacity-70">{m.navAlpha}</a>
            <Link href="/contact" className="hover:opacity-70">{m.navContact}</Link>
          </nav>
      ...
      <section className="mx-auto max-w-[1200px] px-6 pb-20 pt-20 md:pt-28">
        ...
        >
          {m.heroTitle1}<br />
          <span style={{ fontStyle: "italic", color: theme.gold }}>{m.heroTitle2}</span> {m.heroTitle3}
        </h1>
      ...
      <section style={{ background: theme.bgAlt, borderTop: `1px solid ${theme.line}`, borderBottom: `1px solid ${theme.line}` }}>
        <div className="mx-auto grid max-w-[1200px] grid-cols-2 gap-8 px-6 py-12 md:grid-cols-4">
          {[
            { k: m.statLayers, v: "L0–L5" },
            { k: m.statTickers, v: "30+" },
            { k: m.statDepth, v: m.statDepthVal },
            { k: m.statFreq, v: m.statFreqVal },
          ].map((s) => (
            <div key={s.k}>
      ...
      <section id="value-chain" style={{ background: theme.ink, color: theme.bg }} className="px-6 py-24">
      ...
      <footer id="contact" style={{ background: theme.ink, color: theme.bg }} className="px-6 py-16">
        <div className="mx-auto grid max-w-[1200px] gap-10 md:grid-cols-[1.4fr_1fr_1fr]">
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2 text-[14px]" style={{ color: "#d6d0c2" }}>
              <li><Link href="/alpha" className="hover:opacity-70">{m.footerAlpha}</Link></li>
              <li><Link href="/analysis" className="hover:opacity-70">{m.footerAnalysis}</Link></li>
              <li><Link href="/news" className="hover:opacity-70">{m.footerNews}</Link></li>
              <li><Link href="/pricing" className="hover:opacity-70">{m.footerPricing}</Link></li>
            </ul>
          </div>
          <div>
            <p className="text-[11px] tracking-[0.32em]" style={{ color: theme.gold }}>
        ...
        >
          © {new Date().getFullYear()} OPS Capital · {m.footerDisclaimer}
        </div>
      </footer>
    </main>
  );
