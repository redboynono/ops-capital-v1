...
export type FinnhubSocialSentimentPoint = {
  atTime: number;
  mention: number;
...
export type FinnhubSocialSentiment = {
  symbol?: string;
  reddit: FinnhubSocialSentimentPoint[];
  twitter: FinnhubSocialSentimentPoint[];
};
...
export async function fetchSocialSentiment(
  ...
  const url =
    `https://finnhub.io/api/v1/stock/social-sentiment?symbol=${encodeURIComponent(symbol)}` +
    `&from=${fromISO}&to=${toISO}&token=${token()}`;
  try {
    const res = await fetch(url, { cache: "no-store" });
    if (!res.ok) return null;
    const data = (await res.json()) as Partial<FinnhubSocialSentiment>;
    if (!data || (!Array.isArray(data.reddit) && !Array.isArray(data.twitter))) return null;
    return {
      symbol: data.symbol ?? symbol,
      reddit: Array.isArray(data.reddit) ? data.reddit : [],
      twitter: Array.isArray(data.twitter) ? data.twitter : [],
    };
  } catch {
    return null;
  }
