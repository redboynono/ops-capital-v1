...
'
echo "--> db migrations"
for mig in 015_entitlement_brief.sql 016_social_content_type_chokepoint.sql 022_content_feedback.sql 023_ai_weekly_signals.sql 024_x_watch.sql 025_x_watch_reply_zh.sql; do
  sshpass -p "$PASS" ssh -o ServerAliveInterval=15 "$SERVER" \
