...
From repo root locally (deployment credentials are intentionally kept out of git):

```bash
ssh root@188.239.8.157
```

Deploy by syncing the repo to `/opt/ops-alpha`, rebuilding the Docker image, restarting the container, then smoke testing `/ /alpha /login`.

To check logs: `ssh root@188.239.8.157 'docker logs -f --tail 100 ops-alpha'`
